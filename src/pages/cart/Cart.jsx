...
import { addToCart, removeAllFromCart, removeFromItem } from '../../redux/slice/cartSlice'
    ...
    return (
        <div className=''>
            <div className="w-full h-[50vh] relative">
                ...
                <p className='absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-[60px] font-semibold'>Cart</p>
            </div>
                                ...
                                <tr key={item.id} className='border-b pb-3 my-3 flex gap-3 items-center '>
                                    ...
                                    <td className='w-[25%]'>
                                        <div className='flex gap-2 mb-5 justify-center'>
                                            <button onClick={()=> dispatch(addToCart(item))} className='p-[10px] flex justify-center items-center bg-gray-100 border'><FaPlus/></button>
                                            <input type="text" value={item.quantity} className='w-[50px] bg-gray-50 border text-center outline-none' />
                                            <button onClick={()=> dispatch(removeFromItem(item.id))} className='p-[10px] flex justify-center items-center bg-gray-100 border'><FaMinus/></button>
                                        </div>
                                    </td>
                                    ...
                                    <button className=" w-10 h-10 bg-gray-200 flex justify-center items-center rounded-full text-primary">
                                            <IoCloseOutline size={25} onClick={()=> dispatch(removeAllFromCart(item.id))} />
                                    </button>
